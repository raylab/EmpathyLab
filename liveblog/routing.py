...
channel_routing = [
    ...
    route("websocket.connect", connect_blog, path=r'^/(?P<slug>[^/]+)/stream/$'),

    # Called when the client closes the socket
    route("websocket.disconnect", disconnect_blog, path=r'^/(?P<slug>[^/]+)/stream/$'),

    # Called when the client sends message on the WebSocket
    route("websocket.receive", save_post, path=r'^/(?P<slug>[^/]+)/stream/$'),
