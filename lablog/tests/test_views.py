...
class LoanedRecordsByUserListViewTest(TestCase):
    ...
    def setUp(self):
        ...
        test_record = Experiment.objects.create(title='Experiment Title', summary = 'My experiment summary', isbn='ABCDEFG')
        test_record.save();
        ...
        for record_copy in range(number_of_record_copies):
            ...
            status='m'
            Record.objects.create(experiment=test_record,imprint='Unlikely Imprint, 2016', attendant=the_attendant, status=status)
...
class SubjectCreateViewTest(TestCase):
    ...
    def setUp(self):
        ...
        test_user2.user_permissions.add(Permission.objects.get(codename='can_change_status'))
        test_user2.save()
        #Create a experiment
        test_subject = Subject.objects.create(first_name='John', last_name='Smith')
